async function apiRegistro() {
    try {
        ...
        if (response.status === 200) {
            ...
            router("login");
        } else throw new Error(`Error en la respuesta del servidor: ${response.status}`);
    } catch (error) {
        console.error(error);
...
async function apiPeticiones(empresas, fecha, hora) {
    ...
    if (hora != null || hora != undefined) query += `&hora=${hora}`;
    ...
    try {
        ...
        if (response.status !== 200) throw new Error(`Error en la respuesta del servidor: ${response.status}`);
        const json = await response.json();
        return json;
    // return data;
    } catch (error) {
        // throw new Error(`Error en la respuesta del servidor: ${response.status}`);
        console.error("ERROR: Ha ocurrido un error al realizar la petici\xf3n. " + error);
    // console.error(error);
    }
}

//# sourceMappingURL=index.17fa9bca.js.map
