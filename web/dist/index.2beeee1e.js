...
function extraerEmpresas() {
    ...
    if (empresas.length !== 0) {
        let array = [];
        for(let i = 0; i < empresas.length; i++)array.push(empresas[i].id);
        localStorage.setItem("empresas", array);
        router("operar");
        crearEmpresas();
    } else alert("Debe seleccionar alguna empresa para poder operar");
}
...
async function pedir(empresas, fecha, hora) {
    ...
    interval = setInterval(async ()=>{
        ...
        datos.Empresas.map((empresa)=>{
            ...
            if (Number(empresa.cierre) >= preioViejo) document.getElementById(`precio${empresa.empresa}`).style.color = "green";
            else document.getElementById(`precio${empresa.empresa}`).style.color = "red";
            document.getElementById(`precio${empresa.empresa}`).innerHTML = `${empresa.cierre}`;
        });
    }, 10000);
}
...
//# sourceMappingURL=index.2beeee1e.js.map
